...
AESTools::AESTools(const QByteArray &keyqs)
{
    QByteArray key = keyqs;
    if(key.size() != 16)
    {
       char keys[17] = "\0";
        for(int i = 0;i < 16;i++)
...
QByteArray AESTools::Decrypt(const QByteArray &hexData)
{
    ...
    try{
        ...
        QByteArray r2 = QByteArray(da,length - max);
        delete[] da;
        da = NULL;
        return r2;
    }catch(QString exc){
...
QByteArray AESTools::Encrypt(const QByteArray & data)
{
    char* p_data = NULL;
    char *szDataOut = NULL;
    try{
        ...
        memcpy(p_data, data.data(), length);
        //padding块前一部分长度
        int k=length%16;
        //不padding块数
        int j=length/16;
        ...
        QByteArray result = QByteArray(szDataOut, block_num*16);
